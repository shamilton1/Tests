...
> It is assumed that the reader has run the instruction in the [AWS SDAccel README] successfully in order to be a customized to the general flow. 
...
* [Can the kernel/function be optimized with OpenCL attributes or C/C++ directives?](#hls_opt)
...
Examples of code which **does** implement well in hardware are video processing algorithms (HEVC, VP8, etc), encryption and compression standards (Gzip,RSA2048), database acceleration algorithms (e.g. SQL) and general ma thematic and linear algebra functions (GEMM). 
...
In addition, tasks (loops and sub-functions) present in the code may also be executed in parallel. For example, if one iteration of a loop takes N clock cycles and the loop requires J iterations, it will require N*J clock cycles to complete all operations in the loop. If the loop is pipelined in hardware, it will take N+J+1 integrations to complete all operations. If N-10 and J=2M, that is 20,000,000 vs 2,000,011. Code which has many sequential loops and sub-functions benefit greatly from hardware acceleration.

Conversely, code which is simply a few lines of basic operations, and has no tasks (loops or sub-functions) will only obtain acceleration due to parallel logic operations.
